"use client"
// Design intent: Scroll-driven stacked card deck animation for testimonials.
// Tokens: uses shadcn CSS vars (--accent, --accent-foreground, --background, --border).
// States: stacked (initial rotation), animating (translateY + rotate driven by scroll), exited (translateY -180%).
// Context: ContainerScrollContext provides scrollYProgress — CardTransformed MUST be inside ContainerScroll.
// Accessibility: role="article" on each card, aria-labelledby and aria-describedby required at usage site.
// Anti-pattern: never render CardTransformed outside ContainerScroll — throws runtime error.
...
const cardVariants = cva("absolute will-change-transform shadow-[0_8px_30px_rgba(0,0,0,0.12)] dark:shadow-[0_8px_30px_rgba(0,0,0,0.4)]", {
  variants: {
    variant: {
      dark: "flex size-full flex-col items-center justify-center gap-6 rounded-2xl border border-stone-700/50 bg-card p-6",
      light: "flex size-full flex-col items-center justify-center gap-6 rounded-2xl border border-border bg-card p-6",
    },
  },
...
function useContainerScrollContext() {
  ...
  if (context === undefined) {
    throw new Error("useContainerScrollContext must be used within a ContainerScroll (animated-cards-stack)");
  }
  return context;
}
...
export const CardTransformed = React.forwardRef<HTMLDivElement, CardStickyProps>((
  ...
  // Removed expensive dynamic filter calculation for performance
    ...
    <div className={cn("flex items-center gap-2", className)} ref={ref} {...props}>
      <div className="flex items-center">
        ...
        {[...Array(emptyStars)].map((_, i) => (
          <svg key={`empty-${i}`} className="size-4 text-gray-300" fill="currentColor" viewBox="0 0 20 20">
            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.286 3.957a1 1 0 00.95.69h4.162c.969 0 1.371 1.24.588 1.81l-3.37 2.448a1 1 0 00-.364 1.118l1.287 3.957c.3.921-.755 1.688-1.54 1.118l-3.37-2.448a1 1 0 00-1.175 0l-3.37 2.448c-.784.57-1.838-.197-1.54-1.118l1.287-3.957a1 1 0 00-.364-1.118L2.05 9.384c-.783-.57-.38-1.81.588-1.81h4.162a1 1 0 00.95-.69l1.286-3.957z" />
          </svg>
        ))}
      </div>
